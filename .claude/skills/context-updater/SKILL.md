...
- every subsystem note (`docs/internal/*.md`) — one per area, indexed by the root file's *Where the notes live* table; subsystem detail goes there, never into the root file
...
**It is split (#553), and the root file is the only `CLAUDE.md` (#580).** The
root file holds only what every session needs; each subsystem has its own
`docs/internal/<area>.md` (the root file's *Where the notes live* table is the
index). When a change touches one subsystem, update *that* file. The root file
