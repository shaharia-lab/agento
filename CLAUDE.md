...
### macOS signing: ad-hoc, and load-bearing (#461)

`bundle.macOS.signingIdentity` is `"-"`, so `tauri build` runs
`codesign --force -s - --options runtime` over the bundle. **Do not remove it.**
It carries no Apple identity and buys no trust — what it buys is a *valid*
signature, and that is a different thing from a *trusted* one.

Through v1.0.0 nothing signed the bundle. The arm64 artifact still carried the
ad-hoc signature the **linker** applies to every arm64 Mach-O, which seals the
executable and writes no `Contents/_CodeSignature/CodeResources` — so the
bundle's signature declared resources it did not seal. macOS reads that as
**damaged** rather than unsigned, a tampering verdict whose dialog offers only
Move to Trash: no Open Anyway, no Privacy & Security entry, nothing the install
docs described. Every Apple Silicon download of 1.0.0 was unlaunchable.

Why it survived a release, which is the part worth remembering:

- It requires **quarantine**. The executable's own signature is valid, so the app
  ran from CI, from a local `tauri build`, and after every in-app update. Only a
  browser download reproduces it — there is no test-shaped hole here to fill,
  which is why the guard lives in `release.yml` instead.
- **Intel was unaffected**, for the opposite reason: nothing force-signs an
  x86_64 Mach-O, so that bundle was unsigned outright, and unsigned earns the
  ordinary Open Anyway prompt. The two macOS legs failed differently, which is
  how the docs came to describe a path only half the users had.
- `codesign -dv` **exits 0** on the broken bundle and prints a plausible-looking
  ad-hoc signature. Only `codesign --verify --deep --strict` rejects it, and only
  `Identifier=` distinguishes a bundle *we* signed (`com.shaharialab.agento`,
  from `CFBundleIdentifier`) from one the linker signed (`agento-<hash>`). The
  release workflow's verify step checks both on each macOS leg.

`hardenedRuntime` stays at its default `true`; the app is verified to boot, serve
its API and complete a session scan under ad-hoc + hardened runtime. Leaving it
on is also what keeps notarisation from becoming a behaviour change, since
notarisation requires it anyway.
...
update came from us. That is why in-app updates work fine on macOS builds Apple
has never seen — Gatekeeper gates the first launch of a *downloaded* app, not a
bundle the updater swapped in.
