...
on builds Apple has never seen: Gatekeeper gates the first launch of a
downloaded app, not a bundle the updater swapped in.
...
### macOS bundles are ad-hoc signed, and must stay that way

`bundle.macOS.signingIdentity` is `"-"` in `tauri.conf.json`, so `tauri build`
runs `codesign --force -s - --options runtime` over the bundle. That is not an
Apple identity and buys no trust; what it buys is a **valid** signature, and the
distinction is the whole reason the setting exists.

Without it nothing signed the bundle, and the arm64 artifact still carried the
ad-hoc signature the linker applies to every arm64 Mach-O. That covers the
executable and produces no `Contents/_CodeSignature/CodeResources`, so the
bundle's signature declared resources it did not seal. macOS reads that as
**damaged**, not unsigned — a tampering verdict, with only Move to Trash and no
Open Anyway anywhere. Every Apple Silicon download of 1.0.0 was unlaunchable by
the documented route ([#461](https://github.com/shaharia-lab/agento/issues/461)).

Three things kept it hidden for a whole release, all worth knowing before
touching this again:

- It needs **quarantine** to appear. The executable's own signature is valid, so
  the app ran from CI, from a local `tauri build`, and after every in-app update.
  Only a browser download reproduces it.
- **Intel was fine**, for the opposite reason: nothing force-signs an x86_64
  Mach-O, so that bundle was unsigned outright, and unsigned gets the ordinary
  Open Anyway prompt.
- `codesign -dv` **exits 0** on the broken bundle and prints a plausible ad-hoc
  signature. `codesign --verify --deep --strict` is what rejects it, which is
  what the release workflow's verify step runs on both macOS legs.

`hardenedRuntime` is left at its default of `true`, so the executable is signed
with `--options runtime`. Verified to run: the app boots, serves its API and
completes a session scan under an ad-hoc signature with hardened runtime on.
Keeping it on is also what stops notarisation being a behaviour change later —
notarisation *requires* hardened runtime, so turning it off now would mean
turning it back on then, and finding out at that point whether anything breaks.

When a Developer ID does arrive, set `APPLE_SIGNING_IDENTITY` (plus
`APPLE_CERTIFICATE`, `APPLE_CERTIFICATE_PASSWORD`, and `APPLE_ID` +
`APPLE_PASSWORD` + `APPLE_TEAM_ID` for notarisation) in the build step's `env:`.
The env var overrides the config value, so this file does not change. Note that
`APPLE_ID` and `APPLE_PASSWORD` **without** `APPLE_TEAM_ID` is a hard build
failure rather than a skip — all three land together or none do.
