...
// working directory, model, and optional settings profile ID.
func (s *FSChatStore) CreateSession(agentSlug, workingDir, model, settingsProfileID string) (*ChatSession, error) {
	id := newUUID()
	now := time.Now().UTC()
	session := &ChatSession{
		ID:                id,
