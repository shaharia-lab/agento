...
func TestNewSQLiteDB_CreatesTables(t *testing.T) {
	...
	tables := []string{"agents", "chat_sessions", "chat_messages", "integrations", "user_settings", "schema_migrations", "claude_session_cache", "claude_subagent_cache", "claude_cache_metadata", "notification_log", "scheduled_tasks", "job_history", "trigger_rules", "telegram_processed_updates"}
...
func TestNewSQLiteDB_MigrationVersion(t *testing.T) {
	...
	if version != 16 {
		t.Errorf("expected version 16, got %d", version)
	}
}
